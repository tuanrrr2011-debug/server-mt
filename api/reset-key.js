...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const result = await db.collection('keys').updateOne(
      ...
      {
        $set: { deviceId: null, boundAt: null },
      }
    );

    if (result.matchedCount === 0) {
      return res.status(404).json(
        JSON.parse(errorResponse(404, 'License key not found').body)
      );
    }

    return res.status(200).json(
      JSON.parse(successResponse({ message: 'License key reset successfully' }))
    );
  } catch (error) {
